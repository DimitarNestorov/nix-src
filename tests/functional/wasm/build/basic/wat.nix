...
let
  ...
  workspaceVendor = rustPlatform.fetchCargoVendor {
    src = ./.;
    hash = "sha256-f7YATzMDpkxc8W2Mu3jToCgWnbNZGOQ6+yhLpb4cNVc=";
  };
...
rustPlatform.buildRustPackage {
  ...
  buildPhase = ''
    RUSTFLAGS="-L ${wasiSdk}/share/wasi-sysroot/lib/wasm32-wasip1" \
      cargo build --release -p nix-wasm-builder-basic-example \
  ...
  installPhase = ''
    ...
    ${pkgs.lib.getExe pkgs.wasm-tools} print $out/nix_wasm_builder_basic_example.wasm > $out/basic.wat
  '';
