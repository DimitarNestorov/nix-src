...
let
  ...
  workspaceVendor = rustPlatform.fetchCargoVendor {
    src = ./.;
    hash = "sha256-kejIf4R9mh9kN2XSoVvt6zL4G52Am3nUDqrGlMOXDOM=";
  };
...
rustPlatform.buildRustPackage {
  pname = "nix-wasm-builder-argv-test";
  version = "0";
  ...
  buildPhase = ''
    RUSTFLAGS="-L ${wasiSdk}/share/wasi-sysroot/lib/wasm32-wasip1" \
      cargo build --release -p nix-wasm-builder-argv-test \
  ...
  installPhase = ''
    ...
    ${pkgs.lib.getExe pkgs.wasm-tools} print $out/nix_wasm_builder_argv_test.wasm > $out/argv_test.wat
  '';
